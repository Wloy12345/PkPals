...
let tomatoMod; // Variable to store the tomato model

loader.load('tomato.glb', function(gltf) {
  tomatoMod = gltf.scene;
  tomatoMod.rotation.x += 0.5;
  tomatoMod.rotation.y += 0.5;
  scene.add(tomatoMod);
});
...
const canvases = document.querySelectorAll('#tomatoSpin');
...
canvases.forEach((canvas) => {
  ...
  function animate() {
    ...
    // Rotate and move the tomato
    if (tomatoMod) {
      tomatoMod.rotation.y += 0.003;
    }
